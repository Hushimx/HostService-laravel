...
<main id="main-container">
  ...
  <x-hero>
    <x-slot name="title">
      {{ trans('posts.editPost') }} <small class="font-size-base font-w400 text-muted">{{ trans('posts.post') }}</small>
    </x-slot>
    <li class="breadcrumb-item" aria-current="page">
      <a class="link-fx" href="{{{ route('posts.index') }}}">{{ trans('posts.posts') }}</a>
    </li>
    <li class="breadcrumb-item">{{ trans('posts.edit') }} {{ trans('posts.post') }}</li>
  </x-hero>
    ...
    <form action="{{ route('posts.update', $post) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="row">
        ...
        <div class="col-xl-5 mb-3">
          ...
          <input id="post_title" type="text" name="title" class="form-control" value="{{ $post->title }}">
          @error('title')
        ...
        <div class="col-xl-5 mb-3">
          ...
          <select class="custom-select" id="grades" name="grade_id">
            ...
            @foreach ($allGrades as $grade)
              <option value="{{ $grade->id }}" @if ($grade->id == $post->grade_id) selected @endif>{{ $grade->name }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="col-xl-2 mb-3">
          ...
          <div class="custom-control custom-switch mb-1">
            <input type="checkbox" class="custom-control-input" id="post_approve" name="approve" @if ($post->approved) checked @endif>
        ...
        <div class="col-lg-12 mb-4">
          ...
          <textarea class="form-control form-control-alt js-summernote" name="content" id="content" cols="30" rows="4">{{ $post->content }}</textarea>
